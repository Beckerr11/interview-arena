...
const env = z.object({ NODE_ENV:z.enum(["development","test","production"]).default("development"), PORT:z.coerce.number().default(4000), APP_NAME:z.string().default("Interview Arena"), APP_VERSION:z.string().default("1.0.0") }).parse(process.env);
const app = express();
...
app.get("/api/meta", (_req, res) => res.status(200).json({ project:{ name:"Interview Arena", slug:"interview-arena", tagline:"Simulador de entrevista tecnica com treino orientado.", vertical:"edtech" } }));
